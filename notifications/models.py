...
class ApplicationNotification(BaseNotification):
    ...
    """
    application = models.ForeignKey('games.Application', on_delete=models.CASCADE)
...
def game_handle_creation(instance):
    if instance.is_active:
        GameNotification.objects.create(
            game = instance,
            was_reminded = False,
            notification_date_time = instance.date_time - timedelta(days=ADVANCED_NOTIFICATION_DAYS),
            subject = ' '.join([instance.title, 'Game Reminder']),
            message = ' '.join(['Reminder for', instance.title, ':',
                                'Date Time', str(instance.date_time),
    ...
    for field in fields:
        if field in kwargs['update_fields'] and instance.is_active:
            GameNotification.objects.create(**dict(game_dict,
                subject = ' '.join([instance.title, verbose_dict.get(field), 'updated']),
                message = ' '.join([verbose_dict.get(field), 'for', instance.title, 'has been updated to:', str(getattr(instance, field))])
            ))
    if 'is_active' in kwargs['update_fields']:
        if instance.is_active:
            GameNotification.objects.create(**dict(game_dict,
                subject = ' '.join([instance.title, 'uncancelled']),
                message = ' '.join([instance.title, 'has been uncancelled. Here are the details:',
                                    'Date Time', str(instance.date_time),
                                    'Location', instance.location])
            ))
        else:
            GameNotification.objects.create(**dict(game_dict,
                subject = ' '.join([instance.title, 'cancelled']),
...
def notify_application(application):
    ...
    if is_casted(application):
        ApplicationNotification.objects.create(**dict(app_dict,
            subject = ' '.join(['Casted for', application.post.game.title]),
            message = ' '.join(['You are currently now casted for', application.post.game.title])
        ))
    else:
        ApplicationNotification.objects.create(**dict(app_dict,
            subject = ' '.join(['Backup for', application.post.game.title]),
            message = ' '.join(['You are currently now a backup for', application.post.game.title])
        ))
...
def print_application_notification(sender, instance, *args, **kwargs):
    print(instance)
post_save.connect(print_application_notification, sender=ApplicationNotification)
